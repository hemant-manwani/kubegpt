...
from kubegpt.audit import create_printtable_table, run_audit
from kubegpt.chat import run_chat
from kubegpt.diagnose import run_diagnose
from kubegpt.utils import ai_print
from kubegpt.version import __version__

app = typer.Typer(no_args_is_help=True)
console = Console()

kubegpt_prefix = os.getenv("KUBEGPT_PREFIX", "kubectl")
...
def ctl(args: List[str]) -> subprocess.CompletedProcess:
    ...
    kubectl_command = f"{kubegpt_prefix} {kubectl_args}"
...
def diagnose(
    ...
    """
    # run `{kubegpt_prefix} describe` for the resource
    describe_command = f"{kubegpt_prefix} describe {resource_type} {resource_name}"
...
def audit(
    ...
    """
    # run `{kubegpt_prefix} get {resource_type} {resource_name} -oyaml` to get the resource's yaml
    get_yaml_command = f"{kubegpt_prefix} get {resource_type} {resource_name} -oyaml"
...
def chat() -> None:
    """
    Start a chat with kubegpt to generate kubectl commands based your inputs.
    """
    while True:
        ...
        with console.status("[bold green]Generting kubectl command..."):
            command = run_chat(user_prompt).replace("kubectl", kubegpt_prefix)
...
def version_callback(value: bool) -> None:
    if value:
        typer.echo(f"kubegpt version: {__version__}")
        raise typer.Exit()
